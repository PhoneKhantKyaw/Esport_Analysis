SELECT *
FROM Portfolio_Project_Esport.dbo.players;


-- update columns name for players table
...
ALTER TABLE Portfolio_Project_Esport.dbo.country_and_continent DROP COLUMN Three_Letter_Country_Code;
...
ADD country_name_fixed NVARCHAR (255);

UPDATE Portfolio_Project_Esport.dbo.country_and_continent
SET country_name_fixed = CASE
...
SELECT country_name_fixed AS country_name,
    COUNT(*) player_count
FROM Portfolio_Project_Esport.dbo.players players
    JOIN Portfolio_Project_Esport.dbo.country_and_continent country
    ON players.country_code = country.country_code
GROUP BY country_name_fixed
...
SELECT Top 3 country_name_fixed AS country_name,
...
FROM Portfolio_Project_Esport.dbo.players players
    ...
    ON players.country_code = country.country_code
GROUP BY genre
ORDER BY average_prize_money DESC;


-- the most playing game from each continent.
WITH MostPlayingGame AS (
...
RankedPlayerGenreEarnings AS (
    SELECT nickname,
        genre,
        avg_prize_money_per_genre,
        ROW_NUMBER() OVER (
            PARTITION BY genre
            ORDER BY avg_prize_money_per_genre DESC
        ) rank
    FROM PlayerGenreEarnings
)
SELECT nickname,
    genre,
    avg_prize_money_per_genre
FROM RankedPlayerGenreEarnings
WHERE rank <= 5;